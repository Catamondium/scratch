// ORIGINAL SOURCE: https://github.com/eugenp/tutorials/blob/master/core-kotlin/src/main/kotlin/com/baeldung/kotlin/dsl/SqlDsl.kt

/*
fun function(Reciever.() -> Unit) { ... }
             ^^^^^^^^ gives lambda Reciever context
             e.g 'Condition'
             Reciever functions/fields injected to lambda scope
infix fun ...
^^^^^ can be utilised in a context for infix e.g 'eq'
 */
...
fun main(args: Array<String>) {
    val s = query {
        // Functions in 'SqlSelectBuilder' context
        select ("A", "B")
        from   ("C")
        where {
            // Functions in 'Condition' context
            or { // Instance of 'Condition' -> Or
            "A" eq null // Instance "" -> Eq
            "B" eq 5
            }
            "B" eq "STRING"
        }
